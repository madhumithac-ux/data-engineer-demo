...
3. The INSERT statement within the procedure must be identifiable in Snowflake Query History via the tag AI_AGENT_POPULATION_PROC
...
SQL_FILE = Path("C:/Users/MadhumithaC/Projects/data-engineer-demo/src/sql/deb-3_ai_agent_test_table_2.sql")
...
def test_table_exists_in_sql():
    """AC: Table AI_AGENT_TEST_TABLE_2 must exist in schema RAW."""
    assert "CREATE TABLE IF NOT EXISTS RAW.AI_AGENT_TEST_TABLE_2" in _sql(), (
        "CREATE TABLE IF NOT EXISTS statement for RAW.AI_AGENT_TEST_TABLE_2 not found in SQL file"
    )


def test_procedure_exists_in_sql():
    ...
    assert "CREATE OR REPLACE PROCEDURE RAW.P_FILL_AI_AGENT_TEST_TABLE_2" in _sql(), (
        "CREATE PROCEDURE statement for P_FILL_AI_AGENT_TEST_TABLE_2 not found in SQL file"
    )
...
def test_correct_column_types():
    ...
    sql = _sql()
    assert "ID" in sql and "INT" in sql, "Column ID with type INT not found in SQL"
    assert "Name" in sql and "VARCHAR(255)" in sql, "Column Name with type VARCHAR(255) not found in SQL"
    assert "Amount" in sql and "FLOAT" in sql, "Column Amount with type FLOAT not found in SQL"


def test_row_count_in_sql():
...
def test_no_unsafe_drop():
    """AC: SQL must not contain DROP without IF EXISTS (safety check)."""
    unsafe = re.findall(
