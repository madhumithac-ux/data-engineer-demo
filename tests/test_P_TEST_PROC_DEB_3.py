...
def test_query_tag_is_cleared():
    ...
    sql = _proc_sql()
    assert "ALTER SESSION SET QUERY_TAG = '''" in sql or "ALTER SESSION SET QUERY_TAG = '';" in sql, (
        "QUERY_TAG is not cleared after INSERT — expected ALTER SESSION SET QUERY_TAG = '' in procedure SQL"
    )
