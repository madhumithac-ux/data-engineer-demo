...
def test_use_role_present():
    """Table SQL must set the correct role."""
    assert "USE ROLE DEV_SECREF_FR_ENGR_ROLE" in _table_sql(), (
        "USE ROLE DEV_SECREF_FR_ENGR_ROLE not found in table SQL"
    )


def test_use_database_present():
    """Table SQL must set the correct database."""
    assert "USE DATABASE DEV_SECREFTEST_DB" in _table_sql(), (
        "USE DATABASE DEV_SECREFTEST_DB not found in table SQL"
    )
